alter table public.profiles
  ...
  add column if not exists onboarding_step text not null default 'organization',
  add column if not exists onboarding_role text,
  add column if not exists onboarding_work_function text,
  add column if not exists onboarding_use_case text,
  add column if not exists onboarding_tools text[] not null default '{}';
...
alter table public.profiles
  ...
  check (onboarding_step in ('organization', 'name', 'work', 'tools'));
...
set onboarding_step = case
  when p.onboarding_completed then 'tools'
  when p.organization_id is not null and nullif(btrim(coalesce(p.full_name, '')), '') is not null then 'work'
  when p.organization_id is not null then 'name'
  else 'organization'
end
where p.onboarding_step is null
  or p.onboarding_step not in ('organization', 'name', 'work', 'tools');
...
begin
  ...
  set
    ...
    onboarding_completed = public.profiles.onboarding_completed,
    onboarding_step = case
      when public.profiles.onboarding_completed then public.profiles.onboarding_step
      when public.profiles.onboarding_step in ('work', 'tools') then public.profiles.onboarding_step
      else 'name'
    end;
