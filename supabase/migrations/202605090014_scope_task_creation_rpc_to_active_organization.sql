...
begin
  ...
  if v_organization_id is null and p_workspace_id is not null then
    select organization_id into v_organization_id
...
begin
  ...
  v_task := public.create_task_core(
    ...
    p_organization_id := v_recurrence.organization_id
  );
...
comment on function public.create_task_with_recurrence(
  ...
  'Creates an organization-scoped task, its assignees, and task notifications. When recurrence fields are present, it also stores the recurring series metadata.';
