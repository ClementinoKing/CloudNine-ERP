...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      cat <<'USAGE'
Usage:
  ./scripts/deploy-r2-chat-attachments.sh [--skip-secrets]
...
echo "Deploying function: r2-chat-attachments"
supabase functions deploy r2-chat-attachments --project-ref "$PROJECT_REF" --no-verify-jwt
