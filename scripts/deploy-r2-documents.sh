...
for arg in "$@"; do
  case "$arg" in
    ...
    --help|-h)
      cat <<'USAGE'
Usage:
  ./scripts/deploy-r2-documents.sh [--skip-secrets]
...
echo "Deploying function: r2-documents"
supabase functions deploy r2-documents --project-ref "$PROJECT_REF" --no-verify-jwt
